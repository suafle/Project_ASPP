...
df_count = df_center.iloc[int(len(df_center)*rank/size):int(len(df_center)*(1+rank)/size)] 
...
w4ds = np.array(comm.gather(w4dstop, root=0))
gds = np.array(comm.gather(gdstop, root=0))
Gds = np.array(comm.gather(Gdstop, root=0))

if comm.rank==0:
    w4ds = np.concatenate(w4ds).ravel()
    gds = np.concatenate(gds).ravel()
    Gds = np.concatenate(Gds).ravel()
