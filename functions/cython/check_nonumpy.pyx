...
def n_W1_DS(W1,TDS,LStar,eta): #W1 vega mag, TDS in K, LStar in Lsun, TStar in L, eta dimensionless
    #Star magnitude after DS absorption
    cdef list new_w1
    cdef double W1star,Lbol_DS
    new_w1 = []
    #cdef np.ndarray new_w1=np.zeros(len(W1))
    for i in range(len(W1)):
        W1star = W1[i] - 2.5*math.log10(1-eta)
        Lbol_DS=eta*LStar[i] #Lsun
        #W1DS[i] = fw1(TDS,Lbol_DS)[0]
        new_w1.append( -2.5*np.log10(10**(W1star/-2.5) + 10**(fw1(TDS,Lbol_DS)[0]/-2.5)))
    return new_w1#-2.5*np.log10(10**(W1star/-2.5) + 10**(W1DS/-2.5))
