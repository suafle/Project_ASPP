...
def W1_DS(W1,TDS,LStar,eta): #W1 vega mag, TDS in K, LStar in Lsun, TStar in L, eta dimensionless
    ...
    new_w1 = -2.5*np.log10(10**(W1star/-2.5) + 10**(W1DS/-2.5))
    return new_w1


def W2_DS(W2,TDS,LStar,eta): #W1 vega mag, TDS in K, LStar in Lsun, TStar in L, eta dimensionless
    #Star magnitude after DS absorption
    W2star = W2 - 2.5*np.log10(1 - eta)
    Lbol_DS=eta*LStar #Lsun
    W2DS = np.array([fw2(*p)[0] for p in zip(TDS*np.ones(len(W2)),Lbol_DS)])
    new_w2 = -2.5*np.log10(10**(W2star/-2.5) + 10**(W2DS/-2.5))
    return new_w2


def W3_DS(W3,TDS,LStar,eta): #W1 vega mag, TDS in K, LStar in Lsun, TStar in L, eta dimensionless
    #Star magnitude after DS absorption
    W3star = W3 - 2.5*np.log10(1 - eta)
    Lbol_DS=eta*LStar #Lsun
    W3DS = np.array([fw3(*p)[0] for p in zip(TDS*np.ones(len(W3)),Lbol_DS)])
    new_w3 = -2.5*np.log10(10**(W3star/-2.5) + 10**(W3DS/-2.5))
    return new_w3


def W4_DS(W4,TDS,LStar,eta): #W1 vega mag, TDS in K, LStar in Lsun, TStar in L, eta dimensionless
    #Star magnitude after DS absorption
    W4star = W4 - 2.5*np.log10(1 - eta)
    Lbol_DS=eta*LStar #Lsun
    W4DS = np.array([fw4(*p)[0] for p in zip(TDS*np.ones(len(W4)),Lbol_DS)])
    new_w4 = -2.5*np.log10(10**(W4star/-2.5) + 10**(W4DS/-2.5))
    return new_w4
